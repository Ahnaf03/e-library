    ...
    <section class="pt-10 bg-gray-100 sm:pt-16 lg:pt-24">
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
            <div class="max-w-2xl mx-auto text-center">
                <h2 class="text-3xl font-bold leading-tight text-black sm:text-4xl lg:text-5xl lg:leading-tight">Search Your Favorite Book Here</h2>
                <p class="mt-6 text-lg text-gray-900">You can find every book in every country easier.</p>
        ...
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
          <dl class="grid grid-cols-1 gap-x-8 gap-y-16 text-center lg:grid-cols-3">
            <div class="mx-auto flex max-w-xs flex-col gap-y-4">
              ...
              <dd class="order-first text-3xl font-semibold tracking-tight text-gray-900 sm:text-5xl">44 milion</dd>
            </div>
            <div class="mx-auto flex max-w-xs flex-col gap-y-4">
              <dt class="text-base/7 text-gray-600">Assets under holding</dt>
              <dd class="order-first text-3xl font-semibold tracking-tight text-gray-900 sm:text-5xl">$119 bilion</dd>
            </div>
            <div class="mx-auto flex max-w-xs flex-col gap-y-4">
              <dt class="text-base/7 text-gray-600">New users annually</dt>
              <dd class="order-first text-3xl font-semibold tracking-tight text-gray-900 sm:text-5xl">41,000</dd>
            </div>
          </dl>
        </div>
    </div>
        ...
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
            ...
            <div class="grid max-w-md grid-cols-1 mx-auto mt-12 lg:max-w-full lg:mt-16 lg:grid-cols-3 gap-x-16 gap-y-12">
                @foreach ($books as $book)
                    <div>
                        <a href="/hall/{{ $book->slug }}" title="" class="block aspect-w-4 aspect-h-3">
                            @if ($book->cover)
                                <img class="object-cover w-full h-full" src="{{ Storage::url($book->cover) }}" alt="" />
                            @else
                        ...
                        <span class="inline-flex px-4 py-2 text-xs font-semibold tracking-widest uppercase rounded-full {{ $book->category_color }} mt-9"> {{ $book->category->name }} </span>
                        <p class="mt-6 text-xl font-semibold">
                            <a href="/hall/{{ $book->slug }}" title="" class="text-black"> {{ $book->name }} </a>
                        </p>
                        <p class="mt-4 text-gray-600">{{ Str::limit($book->body, 150) }}</p>
                        <div class="h-0 mt-6 mb-4 border-t-2 border-gray-200 border-dashed"></div>
                        <span class="block text-sm font-bold tracking-widest text-gray-500 uppercase"> {{ $book->author->name }} . {{ $book->published_at->format('F j, Y') }} </span>
                    </div>
                @endforeach
            </div>
        </div>
